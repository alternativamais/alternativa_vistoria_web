...
const UploadImagens = ({ vistoriaFerramentaItemId, onSuccess }) => {
  ...
  const handleUploadImagens = async () => {
    if (!vistoriaFerramentaItemId) {
      notification({ message: 'ID da vistoria não foi definido!', type: 'error' });
      return;
    }

    const formData = new FormData();
    // Usa a chave correta para o upload
    formData.append('vistoriaFerramentaItemId', vistoriaFerramentaItemId);
    imagens.forEach((img) => {
    ...
    try {
      await api.post('/imagens-vistoria-ferramentas/upload', formData, {
  ...
  return (
    <Box sx={{ display: 'flex', flexDirection: 'column', gap: 2 }}>
      ...
      <Button variant="contained" color="primary" onClick={handleUploadImagens} disabled={!imagens.length || !vistoriaFerramentaItemId}>
        Enviar Imagens
      </Button>
