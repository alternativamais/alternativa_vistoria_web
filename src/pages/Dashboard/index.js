...
const Dashboard = () => {
  // ==================================================
  // 1) States (todos serão usados para evitar warnings)
  // ==================================================
  ...
  // Tendências (atendimentos e vistorias)
  ...
  // Ao montar o componente, chama tudo
  useEffect(() => {
  ...
  // ===========================================
  // 3) Exemplo de gráficos e uso dos dados
  // ===========================================

  // PieChart: atendimentos por status
  const pieDataAtendimentos = atendimentosStatus.map((item) => ({
    name: item.status,
    value: item.count
  }));

  // BarChart: vistorias por tipo => Usar checklistsPorTipo ou vistoriasPorTipo?
  // Aqui iremos mostrar statusVistorias.statusAgrupado num bar chart, por exemplo.
  const barDataStatusVistorias = (statusVistorias.statusAgrupado || []).map((item) => ({
    name: item.status,
    count: Number(item.count)
  }));

  // LineChart: tendência atendimentos
  const lineDataAtendimentos = (tendenciaAtendimentos || []).map((item) => ({
    name: `Mês ${item.mes}`,
    quantidade: item.quantidade
  }));

  // LineChart: tendência vistorias
  const lineDataVistorias = (tendenciaVistorias || []).map((item) => ({
    name: `Mês ${item.mes}`,
    quantidade: item.quantidade
  }));

  // PieChart: checklists por tipo
  const pieDataChecklistsPorTipo = checklistsPorTipo.map((item) => ({
    name: item.tipo,
    value: item.count
  }));

  // Exemplo de dados para quilometragem (top maiores distâncias)
  // Podemos mostrar em lista
  const topDistancias = quilometragem.topMaioresDistancias || [];

  // Tabela para exibir usuários info
  const { totalUsuarios, ativos, inativos } = usuariosInfo;

  // Exemplo de loop do checklists (pendentes, concluídos etc.)
  ...
  return (
    <Grid container rowSpacing={4} columnSpacing={2.75}>
      ...
      {/* 3) Status Vistorias (BarChart) + Tempo médio (Texto) */}
      <Grid item xs={12} md={6} lg={4}>
        <Card>
          <CardContent>
      ...
      <Grid item xs={12} md={6}>
        <Card>
          <CardContent>
            ...
            <Box height={250}>
              <ResponsiveContainer width="100%" height="100%">
                <LineChart data={lineDataAtendimentos}>
                  <XAxis dataKey="name" />
                  <YAxis />
                  <Tooltip />
      ...
      <Grid item xs={12} md={6}>
        <Card>
          <CardContent>
            ...
            <Box height={250}>
              <ResponsiveContainer width="100%" height="100%">
                <LineChart data={lineDataVistorias}>
                  <XAxis dataKey="name" />
                  <YAxis />
                  <Tooltip />
